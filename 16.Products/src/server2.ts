...
app.get("/api/products/:id", (req, res, next) => {
    ...
    if (!product) {
        const error: HTTError = {
            ...
            message: "Product not found reading products",
        };
...
app.patch("/api/products/:id", (req, res, next) => {
    ...
    if (!product) {
        const error: HTTError = {
            status: 404,
            statusMessage: "Not Found",
            message: "Product not found while updating product",
        };
        return next(error);
    }
...
app.delete("/api/products/:id", (req, res, next) => {
    ...
    if (index === -1) {
        const error: HTTError = {
            status: 404,
            statusMessage: "Not Found",
            message: "Product not found while deleting product",
        };
        return next(error);
    }
...
app.use((err: HTTError, req: Request, res: Response, _next: NextFunction) => {
    console.error(err);
